...
export class LicensesPageComponent implements OnInit {
  ...
  displayedColumns = licensesDisplayedColumns
  defaultColumns = licensesColumns
  ...
  ngOnInit(): void {
    ...
    this.isLoading = true
    this.getAllSub = this.licenseService.getAll(this.searchTerm, this.orderBy, this.pageNumber, this.pageSize)
      .pipe(delay(888))
    ...
    this.licenseService.getAll(this.searchTerm, this.orderBy, 1, 10000000)
      .subscribe((response: LicenseModel[]) => {
        this.licensesCount = response.length
      })
  }
  ...
  remove(id: string) {
    this.deleteSub = this.licenseService.delete(id)
      .subscribe(() => {
        this.licenses = this.licenses.filter(x => x.id != id)
        this.licensesCount = this.licenses.length
      }, error => {
        console.log(error);
      })
  }
  ...
  search() {
    this.searchAllSub = this.licenseService.getAll(this.searchTerm, this.orderBy, this.pageNumber,
  ...
  paging($event: PageEvent) {
    ...
    this.paginateAllSub = this.licenseService.getAll(this.searchTerm, this.orderBy, this.pageNumber,
      this.pageSize, this.filterBy, this.filterByValue)
      .subscribe((response: LicenseModel[]) => {
        this.licenses = response
      })
  ...
  sortData($event: Sort) {
    ...
    this.licenseService.getAll(this.searchTerm, this.orderBy, this.pageNumber,
      this.pageSize, this.filterBy, this.filterByValue)
      .subscribe((response: LicenseModel[]) => {
        this.licenses = response
      })
  ...
  filtering(field: string, filterBy: string) {
    ...
    this.getAllSub = this.licenseService.getAll(this.searchTerm, this.orderBy, this.pageNumber,
      this.pageSize, this.filterBy, this.filterByValue)
      .subscribe((response: LicenseModel[]) => {
        this.licensesCount = response.length
        this.licenses = response
